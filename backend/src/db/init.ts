...
export function initDatabase() {
  ...
  const [{ total }] = db.select({ total: count() }).from(products).all() as [{ total: number }];
  if (total > 0) {
    console.log(`Database already seeded (${total} products)`);
    return;
  }
  ...
  console.log(`Seeded ${insertedCategories.length} categories and ${productData.length} products`);
}
